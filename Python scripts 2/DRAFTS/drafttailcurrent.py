...
def analyze_tail(time, trace, trough, HypEnd, sweep, SampleRate):
    ...
    params, cv = scipy.optimize.curve_fit(monoExp, time[mask], trace[mask], p0=(1, -1, 1))
    a, b, c = params
    ...
    x_fit = np.linspace(time[mask].min(), time[mask].max(), 100000)  # x values for line of best fit
...
plt.plot(x, y, label=f"y = {mean_a:.2f} * exp({mean_b:.2f} * x) + {mean_c:.2f}", color='red')
