...
#to calculate averages of max amplitude:
maxamps_control = []
maxamps_treat = []
maxamps_washout = []
...
# Denoise the trace
        denoised_trace = low_pass_filter(trace, SampleRate)
                ...
                # Filter out outliers
        threshold_z = 2
        denoised_trace = removeOutliersNormalized(denoised_trace, threshold_z)

        # Ensure lengths are consistent
        min_length = min(len(denoised_trace), len(time), len(inputv))
        denoised_trace = denoised_trace[:min_length]
        time = time[:min_length]
        inputv = inputv[:min_length]
        ...
        # Create a mask for the depolarized region
        ...
        index_of_pk = np.argmax(filtered_values2)
        ...
        print("Area Under Curve:", areaundercurve_peak)
        print("Voltage at Peak:", vAtPeak)
        print("Current Peak:", currpeak)
